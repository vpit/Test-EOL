...
sub _all_files {
    ...
    my @found;
    my $want_sub = sub {
        my @chunks = File::Spec->splitdir($File::Find::dir);
        for (0 .. 1) {
            $chunks[$_] = '' unless defined $chunks[$_];
        }
        return if ($chunks[0] eq 'CVS' or $chunks[0] eq '.svn'); # Filter out cvs or subversion dirs
        return if ($chunks[0] eq 'blib' and $chunks[1] eq 'libdoc'); # Filter out pod doc in dist
        return if ($chunks[0] eq 'blib' and $chunks[1] =~ /^man[0-9]$/); # Filter out pod doc in dist
        return if ($chunks[0] eq 'inc'); # Filter out Module::Install stuff
